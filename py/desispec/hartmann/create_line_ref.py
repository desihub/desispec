...
def fit_arc(file_raw,psf_file,channel,dz,ee=0.90,display=False)
    ee=0.90
    channel='B2'
    psf_file='/project/projectdirs/desi/users/jguy/teststand/20181023/psf-b2.fits'
    file_raw='/project/projectdirs/desi/spectro/teststand/rawdata/SM03/2018/20181013/WINLIGHT_00009777.fits'
    ...
    dz=0.
    FWHM_estim=3. #FWHM_estim = lambda z: abs(z) / 2.0 / 1.7 / pix_sz + 3.
    ...
    for i in range(psf.nspec):
        ...
        for j in range(n_line):
            ...
            if np.max(subim)>thres:
                (A, xcentroid, ycentroid, FWHMx, FWHMy,chi2) = psf_tool.PSF_Params(subim, sampling_factor=20.0, display=False, \
