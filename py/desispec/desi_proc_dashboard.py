...
def main(args):
    if args.not os.getenv('DESI_SPECTRO_REDUX'): # these are not set by default in cronjob mode.
    ...
    for night in nights:
        #print(night)
        stat_night=calculate_one_night(night)
        ####################################
        ### Table for individual night ####
        ####################################
        strTable+= nightly_table(stat_night,str(night))
...
def nightly_table(table,night):
    ...
    strTable += "<table id='c'><tr><th>Expid</th><th>FLAVOR</th><th>OBSTYPE</th><th>EXPTIME</th><th>SPECTROGRAGHS</th>"
    strTable += "<th>PSF File</th><th>FFlat file</th><th>frame file</th><th>sframe file</th><th>sky file</th>"
    strTable += "<th>cframe file</th><th>slurm file</th><th>log file</th></tr>"
    ...
    for i in range(len(table)):
        ...
        if obstype in totals_by_type.keys():
            ...
            n_tots = totals_by_type['ZERO']

        n_spgrph=int(table[expid]['n_spectrographs'])

        elements = [expid,table[expid]['FLAVOR'],table[expid]['OBSTYPE'],table[expid]['EXPTIME'],\
                    table[expid]['SPECTROGRAPHS'], \
                    _str_frac(table[expid]['n_psf'],n_spgrph*n_tots['psf']),\
                    _str_frac(table[expid]['n_ff'],n_spgrph*n_tots['ff']),\
                    _str_frac(table[expid]['n_frame'],n_spgrph*n_tots['frame']),\
                    _str_frac(table[expid]['n_sframe'],n_spgrph*n_tots['sframe']),\
                    _str_frac(table[expid]['n_sky'],n_spgrph*n_tots['sframe']),\
                    _str_frac(table[expid]['n_cframe'],n_spgrph*n_tots['sframe']),\
                    _hyperlink('./here.txt','Slurm'),\
                    _hyperlink('./there.txt','Log')]
        strTable += _table_row(elements)
