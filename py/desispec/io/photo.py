...
def tractorphot_datamodel(from_file=False, dr10=False):
    """Initialize the tractorphot data model for DR9 or DR10 photometry.
    ...
    if from_file:
        if dr10:
            ...
            dr = 'dr9'
        datamodel_file = os.environ.get('DESI_ROOT')+f'/external/legacysurvey/{dr}/south/tractor/000/tractor-0001m002.fits'
        datamodel = Table(fitsio.read(datamodel_file, rows=0, upper=True))
        for col in datamodel.colnames:
            datamodel[col] = np.zeros(datamodel[col].shape, dtype=datamodel[col].dtype)
...
def gather_tractorphot(input_cat, racolumn='TARGET_RA', deccolumn='TARGET_DEC',
    ...
    Args:
        ...
        legacysurveydir (str): full path to the location of the DR9 Tractor catalogs
    ...
    if 'dr10' in legacysurveydir:
        datamodel = tractorphot_datamodel(dr10=True)
    else:
        datamodel = tractorphot_datamodel(dr10=False)
