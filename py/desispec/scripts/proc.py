...
def main(args=None, comm=None):
    ...
    if not (args.obstype in ['SCIENCE'] and args.noprestdstarfit):
        ...
        for i in range(rank, len(args.cameras), size):
            ...
            cmd = desispec.scripts.preproc.main
            try:
                err = runcmd(cmd, args=[preproc_args], inputs=[args.input], outputs=[outfile])
            except Exception:
                err = True
            if err != 0:
                error_count += 1
    ...
    if args.obstype in ['FLAT', 'TESTFLAT'] :
        ...
        if exptime > 10:
            ...
            for i in range(rank, len(args.cameras), size):
                ...
                cmd = desispec.scripts.fiberflat.main
                try:
                    err = runcmd(cmd, args=[fiberflat_args], inputs=[framefile,], outputs=[fiberflatfile,])
                except Exception:
                    err = True
                if err != 0:
                    error_count += 1
    ...
    if args.obstype in ['SCIENCE', 'SKY'] and (not args.noskysub ) and (not args.noprestdstarfit):
        ...
        for i in range(rank, len(args.cameras), size):
            ...
            cmd = desispec.scripts.sky.main
            try:
                err = runcmd(cmd, args=[sky_args], inputs=[framefile, fiberflatfile], outputs=[skyfile,])
            except Exception:
                err = True
            if err != 0:
                error_count += 1
    ...
    if args.obstype in ['SCIENCE'] and \
        ...
        for camera in args.cameras[rank::size]:
            ...
            cmd = desispec.scripts.fluxcalibration.main
            try:
                err = runcmd(cmd, args=[fluxcal_args], inputs=inputs, outputs=[calibfile,])
            except:
                err = True
            if err != 0:
                error_count += 1
    ...
    if args.obstype in ['SCIENCE',] and (not args.noskysub ) and (not args.nofluxcalib) :
        ...
        for camera in args.cameras[rank::size]:
            ...
            cmd = desispec.scripts.procexp.main
            try:
                err = runcmd(cmd, args=[procexp_args], inputs=inputs, outputs=[cframefile,])
            except Exception:
                err = True
            if err != 0:
                error_count += 1
