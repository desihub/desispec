...
from desispec.workflow.proc_dashboard_funcs import get_skipped_expids, \
    return_color_profile, _initialize_page, js_str, _closing_str, monthly_table, \
    _table_row, find_new_exps, _hyperlink, _str_frac
...
def main(args=None):
    ...
    if args.nights is None or args.nights == 'all' or ',' not in args.nights:
        nights = list()
        for n in listdir(
                os.path.join(args.prod_dir, 'run', 'scripts', 'night')):
            # - nights are 20YYMMDD
    ...
    for month, nights_in_month in nights_dict.items():
        ...
        for night in nights_in_month:
            ...
            nightly_tables.append(
                nightly_table(night, args.output_dir, skipd_expids,
    ...
    outfile = os.path.abspath(
        os.path.join(os.environ['DESI_DASHBOARD'], args.output_name))
...
def nightly_table(night, output_dir, skipd_expids=set(), show_null=True,
                  check_on_disk=False, ignore_json_archive=False):
    """
    ...
    filename_json = os.path.join(output_dir, 'files',
                                 'night_info_' + os.environ[
                                     'SPECPROD'] + '_' + night + '.json')
...
def calculate_one_night_use_file(night, check_on_disk=False,
                                 night_info_pre=None):
    """
    ...
    for row in d_exp:
        ...
        output[str(expid)] = [row_color,
                              ...
                              _str_frac(nfiles['frame'],
                                        ncams * expected['frame']),
                              _str_frac(nfiles['ff'], ncams * expected['ff']),
                              _str_frac(nfiles['sframe'],
                                        ncams * expected['sframe']),
                              _str_frac(nfiles['sky'],
                                        ncams * expected['sframe']),
                              _str_frac(nfiles['std'],
                                        nspecs * expected['std']),
                              _str_frac(nfiles['cframe'],
