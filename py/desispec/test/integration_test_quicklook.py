...
def parse(options=None):
    ...
    expid = 2
    sim_dir = os.path.join(os.environ['DESI_SPECTRO_SIM'],'exposures',args.night)
    output_dir = os.path.join(os.environ['DESI_SPECTRO_REDUX'],'ql_output')
    ...
    if args.overwrite:
        ...
        if os.path.exists(output_dir):
            expo_dir = os.path.join(output_dir,'exposures')
            night_dir = os.path.join(expo_dir,args.night)
            exp_dir = os.path.join(night_dir,'{:08d}'.format(expid))
        ...
        if os.path.exists(sim_dir) or os.path.exists(output_dir):
            raise RuntimeError('Files already exist for this night! Can overwrite if necessary')
...
def integration_test(args=None):
    ...
    args = parse(args)
    datadir = os.path.join(os.environ['DESI_SPECTRO_SIM'],'exposures')
    output_dir = os.path.join(os.environ['DESI_SPECTRO_REDUX'],'ql_output')
    ...
    for camera in ['r0','z0']:
        ...
        com = "desi_quicklook -n {} -c {} -e {} -f {} --psfboot {} --fiberflat {} --rawdata_dir {} --specprod_dir {}".format(night,camera,expid,'dark',psffile,fiberflatfile,datadir,output_dir)
        if runcmd(com) != 0:
    ...
    if args.delete:
        sim_dir = os.path.join(datadir,night)
        expo_dir = os.path.join(output_dir,'exposures')
        night_dir = os.path.join(expo_dir,night)
        exp_dir = os.path.join(night_dir,'{:08d}'.format(expid))
        qa_dir = os.path.join(exp_dir,'qa')
        sim_files = os.listdir(sim_dir)
        for file in range(len(sim_files)):
