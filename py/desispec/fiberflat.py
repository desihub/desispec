...
def compute_fiberflat(frame, nsig_clipping=4., accuracy=1.e-4) :
    ...
    log.info("1st pass ignoring resolution, median spectrum")
    ...
    # add 1% syst. error on flux in ivar to limit problems on very bright signal region
    #for fiber in range(nfibers) :
    #    ok=np.where((mean_spectrum>0)&(ivar[fiber]>0))[0]
    #    if ok.size >0 :
    #        ivar[fiber,ok]=1./( 1./ivar[fiber,ok] + 0.01**2*mean_spectrum[ok]**2)
    
    log.info("after 1st pass : nout = %d/%d"%(np.sum(ivar==0),np.size(ivar.flatten())))
    log.info("2nd pass ignoring resolution")
    ...
    for iteration in range(max_iterations) :
        ...
        log.info("2nd pass, iter %d"%iteration)
        ...
        smooth_fiberflat_inv=np.zeros(smooth_fiberflat.shape)
        for fiber in range(nfibers) :
            ok=np.where(smooth_fiberflat[fiber]!=0)[0]
            smooth_fiberflat_inv[fiber,ok]=1./smooth_fiberflat[fiber,ok]
        swx=np.sum(ivar*flux*smooth_fiberflat_inv,axis=0)
        sw=np.sum(ivar*smooth_fiberflat_inv,axis=0)
        mean_spectrum=np.zeros((sw.size))
        ok=np.where(sw!=0)[0]
        mean_spectrum[ok]=swx[ok]/sw[ok]
        ...
        if max_diff>0.01 :
            log.info("2nd pass, max diff. = %g > 0.01 , continue iterating before outlier rejection"%(max_diff))
            continue
                    

        nout_iter=0
        if True : 
            nsig_clipping_for_this_pass = 3.
            
            if iteration>0 and iteration<10 :
                # for first iteration, the mean spectrum is a median so it's robust
                #
                # then the mean spectrum can be affected by outliers so we 
                # have to be cautious before rejecting many pixels
                #
                ...
                nout_per_wave=np.sum(chi2>nsig_clipping_for_this_pass**2,axis=0)
                selection=np.where(nout_per_wave>0)[0]
                for i in selection :
                    worst_entry=np.argmax(chi2[:,i])
                    ivar[worst_entry,i]=0
                    nout_iter += 1

            else :
                # remove all of them at once
                bad=(chi2>nsig_clipping_for_this_pass**2)
                ivar *= (bad==0)
            ...
            log.info("2nd pass iter #%d chi2=%f ndf=%d chi2pdf=%f nout=%d (nsig=%f)"%(iteration,sum_chi2,ndf,chi2pdf,nout_iter,nsig_clipping_for_this_pass))
        ...
        if nout_iter == 0 :
            break
    log.info("after 2nd pass : nout = %d/%d"%(np.sum(ivar==0),np.size(ivar.flatten())))
    
    for iteration in range(100) : 
        
        log.info("3rd pass, iter %d : mean deconvolved spectrum"%iteration)
        ...
        if True :
            ...
            for fiber in range(nfibers) :
                if fiber%10==0 :
                    log.info("3rd pass, filling matrix, iter %d fiber %d"%(iteration,fiber))

                    ### R = Resolution(resolution_data[fiber])
                    R = frame.R[fiber]
                    #R = np.eye(nwave).astype(float)
        ...
        chi2=ivar*(flux-smooth_fiberflat*mean_spectrum)**2
        print "chi2/ndata=",np.sum(chi2)/(np.sum(chi2>0))
        
        # normalize to get a mean fiberflat=1
        #mean=np.mean(smooth_fiberflat,axis=0)
        mean=np.zeros((smooth_fiberflat.shape[1]))
        swx=np.sum(ivar*smooth_fiberflat,axis=0)
        sw=np.sum(ivar,axis=0)
        ok=np.where(sw!=0)[0]
        mean[ok]=swx[ok]/sw[ok]
        ...
        # we don't start the rejection tests until we have converged on this
        if max_diff>0.01 :
            log.info("3rd pass, iter %d, max diff. = %g > 0.01 , continue iterating before outlier rejection"%(iteration,max_diff))
            continue
        ...
        if ndf>0 :
            chi2pdf=sum_chi2/ndf
        log.info("3rd pass, iter %d, chi2=%f ndf=%d chi2pdf=%f nout=%d"%(iteration,sum_chi2,ndf,chi2pdf,nout_iter))

        
        if max_diff>accuracy :
            log.info("3rd pass, iter %d, max diff. = %g > requirement = %g , continue iterating"%(iteration,max_diff,accuracy))
            continue
    ...
    log.info("4th pass, final computation of fiber flat")
    ...
    for fiber in range(nfibers) :
        ...
        smooth_fiberflat=spline_fit(wave,wave,fiberflat[fiber],smoothing_res,ivar[fiber]*M**2*(M!=0))
